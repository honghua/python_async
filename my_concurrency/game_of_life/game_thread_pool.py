...
class ColumnPrinter:
    ...
    def __str__(self):
        ...
        rows = [""] * row_count
        for j in range(row_count): # j:= 1, 2, ..., rows
...
def simulate_pool(grid, pool: ThreadPoolExecutor):
    next_grid = Grid(grid.height, grid.width)
    futures = []
    for y in range(grid.height):
        for x in range(grid.width):
...
def simulate_threaded(grid):
    ...
    for y in range(grid.height):
        for x in range(grid.width):
            thread = threading.Thread(target = step_cell, args=(y, x, grid.get, next_grid.set))
            thread.start()
...
def main():
    ...
    with ThreadPoolExecutor(45) as ex:
        for i in range(5):
            columns.append(str(grid))
            grid = simulate_pool(grid, ex)
    print(columns)
...
if __name__ == '__main__':
    ...
    main()
    te = time.time()
    LOG.info(f"took {te - ts: .2f} seconds")